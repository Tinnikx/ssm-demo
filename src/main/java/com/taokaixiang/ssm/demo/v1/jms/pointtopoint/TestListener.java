    ...
    public void onMessage(Message message) {
        try{
            if (message instanceof BytesMessage) {
                ...
                System.out.println(TestListener.class.getName() + " 收到的消息： " + map);
            } else if (message instanceof TextMessage)
                System.out.println(TestListener.class.getName() + " 收到的消息： "+((TextMessage)message).getText());
            else
                System.out.println(TestListener.class.getName() + " 收到一个未识别的消息: " + message.toString());
